...
x0 = 0                               # нулевая координата
y0 = 0
angin = 30.0                         # угол входной кромки
angout = 15.0                        # угол выходной кромки
t = 1.2                              # абсолютный шаг

# центральная линия по углам потока
B = curve.curve(x0, y0, b, beta, gamma, n)

# координаты начала и конца профиля лопатки
spvh = np.array([B[0, 0]+r[0]*np.cos(np.pi/2 - np.deg2rad(beta[0] - angin / 2.0)),
                 B[0, 1]-r[0]*np.sin(np.pi/2 - np.deg2rad(beta[0] - angin / 2.0))])
spvih = np.array([B[-1, 0]+r[1]*np.cos(np.pi/2 - np.deg2rad(beta[1] - angout / 2.0)),
                  B[-1, 1]+r[1]*np.sin(np.pi/2 - np.deg2rad(beta[1] - angout / 2.0))])
corvh = np.array([B[0, 0]-r[0]*np.cos(np.pi/2 - np.deg2rad(beta[0] + angin / 2.0)),
                  B[0, 1]+r[0]*np.sin(np.pi/2 - np.deg2rad(beta[0] + angin / 2.0))])
corvih = np.array([B[-1, 0]-r[1]*np.cos(np.pi/2 - np.deg2rad(beta[1] + angout / 2.0)),
                   B[-1, 1]-r[1]*np.sin(np.pi/2 - np.deg2rad(beta[1] + angout / 2.0))])
...
# вторая лопатка
circle3 = plt.Circle((B[0, 0] + t, B[0, 1]), r[0])
circle4 = plt.Circle((B[-1, 0] + t, B[-1, 1]), r[1])
